...
public class HypervolumeCalculator {
    public static void main(String[] args) throws IOException {
        ...
        Arrays.fill(MAXIMUM, 3);
        Arrays.fill(MINIMUM, 0);

        ArrayList<double[]> payoffs = load_payoff( "bestSolution/GA/PayoffSolutionGA.txt");
//        ArrayList<double[]> payoffs = load_payoff("bestSolution/Tabu/ParetoSolutionTabu.txt");
        System.out.println(payoffs.size());

//        ArrayList<double[]> pareto = findPareto(payoffs);
//        savePareto("bestSolution/GA/ParetoSolutionGA.txt", pareto);
//        System.out.println("Pareto Size: " + pareto.size());
        double hypervolume = hypervolume_calculator(payoffs, MINIMUM, MAXIMUM);
...
//        for (double[] candidate : payoffs) {
//            boolean isDominated = false;
//            if (contain(pareto, candidate)) {
//                continue;
//            }
//            for (double[] other : payoffs) {
//                if (dominates(other, candidate)) {
//                    isDominated = true;
//                    break;
//                }
