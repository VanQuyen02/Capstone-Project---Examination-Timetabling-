...
//                && passConstraint7()
    ...
    public void getFitness() {
        if (!passAllConstraint()) {
            ...
            getPayoffAllM();
            getPayoffAllI();
            fitness = w1 * payoffP + w2 * payoffAllM + w3 * payoffAllI;
        }
    }
...
//        System.out.println("Number of day: " + numberOfDay + "Number of slot: " + Math.abs(numberOfSlot - data.Q[i]));
        return w4 * numberOfDay + w5 * Math.abs(numberOfSlot - data.Q[i]);
    }
    ...
    public void writePayoff(String sheetName) throws IOException {
        ...
        writer.write(1.0 / 20.0 * payoffP + " " + 4.0 / 20.0 * payoffAllM + " " + 15.0 / 20.0 * payoffAllI + "\n");
        writer.close();
    }
